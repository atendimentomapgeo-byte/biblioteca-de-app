...
(function () {
  ...
  const APP_BUILD_VERSION = 'v57-debug-cores';
  ...
  function wireGPS() {
    GPS.on((event, data) => {
      if (event === 'position') {
        ...
        // Atualiza a declinação magnética pra posição atual (usada por
        // wireCompass pra converter o rumo da bússola, que é magnético, para
        // o Norte verdadeiro usado pelo mapa). Recalculado aqui — a cada
        // atualização do GPS — em vez de a cada leitura da bússola (que
        // chega com frequência bem maior), já que a declinação muda pouco
        // com pequenos deslocamentos.
        if (window.Declination) {
          try {
            declinacaoAtual = Declination.getDeclination(data.lat, data.lon, (data.altitude || 0) / 1000);
          } catch (e) { /* mantém o último valor válido em caso de erro pontual */ }
        }
  ...
  function wireCompass() {
    ...
    Compass.on((event, data) => {
      if (event === 'heading') {
        // A bússola do aparelho lê o Norte MAGNÉTICO — precisa da correção
        // de declinação pra alinhar com o Norte VERDADEIRO usado pelo mapa
        // e pelas coordenadas GPS (rumoVerdadeiro = rumoMagnético + declinação).
        // Sem isso, o mapa girado pela bússola fica sistematicamente
        // deslocado da realidade (no Brasil, tipicamente 15° a 24° a Oeste).
        let rumo = data.heading;
        if (declinacaoAtual != null) {
          rumo = ((rumo + declinacaoAtual) % 360 + 360) % 360;
        }
        ...
        MapModule.setMapRotation(rumo);
        const label = $('compass-heading-label');
        if (label) label.textContent = Math.round(rumo) + '°';
    ...
    btn.onclick = async () => {
      ...
      // Calcula a declinação já com a posição atual, sem esperar a próxima
      // leitura do GPS chegar (evita o mapa girar sem correção por alguns
      // segundos logo depois de ativar).
      if (declinacaoAtual == null && window.Declination) {
        const posAtual = GPS.getLastPosition();
        if (posAtual) {
          try {
            declinacaoAtual = Declination.getDeclination(posAtual.coords.latitude, posAtual.coords.longitude, (posAtual.coords.altitude || 0) / 1000);
          } catch (e) { /* segue sem correção até a próxima posição chegar */ }
        }
      }
