  ...
  const MapModule = {
    ...
    setBaseLayer(name) {
      ...
      if (layer) {
        ...
        // Trocou o mapa base: a cor amostrada da borda inferior (edgecolor.js)
        // era de outra camada e ficaria errada até o próximo movimento do
        // mapa — atualiza já, se o módulo estiver carregado.
        if (window.EdgeColor) window.EdgeColor.refresh();
      }
    },
    ...
    /** Devolve o objeto Leaflet da camada base pelo nome (usado por edgecolor.js para ler getTileUrl). */
    getBaseLayerObject(name) {
      return baseLayers[name] || null;
    },
